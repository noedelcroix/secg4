...
public class BasicServer implements AutoCloseable
{
    ...
    {
        void handle(
                NonReplayableMessage m, ObjectInputStream in, ObjectOutputStream out)
                throws IOException, NoSuchAlgorithmException, InvalidKeySpecException, NoSuchPaddingException, IllegalBlockSizeException, BadPaddingException, InvalidKeyException;
    }
    ...
    private class BasicServerThread extends Thread
    {
        ...
        public void run()
        {
            ...
            while (!exit)
            {
                try
                {
                    NonReplayableMessage m = (NonReplayableMessage) messageReader.readObject();
                    System.out.println(
