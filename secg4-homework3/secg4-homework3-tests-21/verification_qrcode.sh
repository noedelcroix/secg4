...
for userimg in "$@"
    do
    mkdir tmp
    user=$(zbarimg --raw -q $userimg)
    echo $user
    #retrieve all the data needed like core, fingerprint, signature, the dates and stock them into a temporary directory tmp
    echo $user | sed -n '1,7p' > tmp/dpse_data.txt
    echo $user | sed -n '8p' | base64 --decode > tmp/dpse_signature.txt
    echo $user | sed -n '9p' > tmp/dpse_fingerprint.txt
    id=$(echo $user | sed -n '3p');
    ...
    if [ $? -ne 0 ]; then
        ...
        startDate=$(echo $user | sed -n '6p');
        endDate=$(echo $user | sed -n '7p');
        today=$(date +%s);
